...
Escolha do modelo:
	Gemini 2.5 Flash	Claude (Anthropic)	GPT-4o (OpenAI)	Ollama (local)
...
4. Gemini API
Acesse aistudio.google.com, gere uma API key e insira no node "Requisita a IA" (HTTP Request) substituindo o placeholder na URL.
Importante: nunca commite a API key no repositório.
5. Google Sheets
O workflow utiliza o Google Sheets como destino dos dados extraídos. Para conectar o n8n à sua conta Google, é necessário criar credenciais OAuth2 no Google Cloud Console:
...
•	Google Gemini 2.5 Flash (classificação e extração via IA)
•	Google Sheets (output)
•	Flask / Python (imap-service)
